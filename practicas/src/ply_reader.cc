...
void leer_cabecera( ifstream & src,
                    unsigned & num_v, unsigned & num_caras,
                    const bool lee_num_caras ) ;
void error( const char *msg_error ) ;
void leer_v(  unsigned num_v, vector<Tupla3f> & v,
                     ifstream & src  ) ;
void leer_caras( unsigned num_v,
...
)
{
   ...
   leer_cabecera( src, num_v, num_caras, true ) ;
   leer_v( num_v, v, src ) ;
   leer_caras( num_v, num_caras, caras, src ) ;

   cout << "archivo ply '" << na << "' leido: núm. vértices == " << num_v << ", núm caras == " << num_caras << endl << flush ;
}
...
)
{
   ...
   abrir_archivo( na, src ) ;
   leer_cabecera( src, num_v, num_caras, false ) ;
   leer_v( num_v, v, src ) ;

   cout << "archivo ply '" << na << "' leido: núm. vértices == " << num_v << " (caras no leídas)." << endl << flush ;
}
...
)
{
   ...
   //cout << "  leyendo " << num_v << " vértices ...." << endl << flush ;

   for( long iv = 0 ; iv < num_v ; iv++ )
   {
      if ( src.eof() )
