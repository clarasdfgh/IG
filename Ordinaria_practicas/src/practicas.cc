...
//***************************************************************************
// Funcionalidades prácticas 4 y 6
//
//***************************************************************************

void funcion_idle() {
   if(escena != 0)
      escena->animarAutomatico();
   glutPostRedisplay();
}

void clickRaton(int boton, int estatus, int x, int y) {
   if(escena != 0)
      escena->clickRaton(boton, estatus, x, y);
   glutPostRedisplay();
}
...
int main( int argc, char **argv )
{
   ...
   // P1: SUSTITUIR EL NOMBRE DEL ALUMNO
   glutCreateWindow("Practicas IG: CLARA ROMERO");
   ...
   //P4: Función IDLE para la animación automática
   glutIdleFunc(funcion_idle);

   //P6: Movimiento de la cámara con el ratón
